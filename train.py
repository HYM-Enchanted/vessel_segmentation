...
def main():
    ...
    torch.manual_seed(args.seed)  #为当前cpu设置种子用于生成随机数，torch.cuda.manual_seed()为当前GPU设置……
    if args.cuda:
    ...
    model = nn.parallel.DataParallel(model, device_ids=gpu_ids)   #pytorch支持多gpu训练

    if args.resume:
        if os.path.isfile(args.resume):   #是否从设置断点处继续训练，args.resume是存放最好模型的位置
            print("=> loading checkpoint '{}'".format(args.resume))
            checkpoint = torch.load(args.resume)  #重新加载这个模型
    ...
    for epoch in range(1, args.nEpochs + 1):
        ...
        train(args, epoch, model, trainLoader, optimizer, trainF, class_weights)   #训练
        err = test(args, epoch, model, testLoader, optimizer, testF, class_weights)  #测试
        is_best = False
        if err < best_prec1:   #如果模型更好，则保存这个模型
            is_best = True
            best_prec1 = err
...
def train_nll(args, epoch, model, trainLoader, optimizer, trainF, weights):
    ...
    for batch_idx, (data, target) in enumerate(trainLoader):   #trainloader里面包含data和target
        if args.cuda:
